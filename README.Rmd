...
gt1 <- gamtree(Pn ~ s(noise) | Species, data = eco, verbose = FALSE, 
               cluster = eco$specimen, mob_ctrl = mob_control(maxdepth = 2L))
sctest(gt1$tree)

## What happens if the predictor is measured at the observation level, 
## and we average the noise variable within cluster?

eco2 <- eco
eco2$noise <- rep(tapply(eco$noise, eco$specimen, mean), times = table(eco$specimen))
gt2 <- gamtree(Pn ~ s(noise) | Species, data = eco2, verbose = FALSE, 
               cluster = eco2$specimen,
